...
public class GameRotationVectorController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        wValue = sensorValueLayout.transform.Find("SensorWValue").gameObject.GetComponent<Text>();
    ...
    void Update()
    {
        ...
        var value = MoverioInput.GetGameRotation();
        UpdateSensorValue(value);

        try
        {
            var accuracy = MoverioInput.GetGameRotationAccuracy();
            UpdateSensorAccuracy(accuracy);
        }
        catch (IOException e)
        {
            Debug.LogError("Getting the accuracy of Game Rotation vector sensor is failed. Message = " + e.Message);
        }
    }

    private void UpdateSensorValue(Quaternion value)
    {
        xValue.text = value.x.ToString("F4");
        yValue.text = value.y.ToString("F4");
        zValue.text = value.z.ToString("F4");
        wValue.text = value.w.ToString("F4");
    }
