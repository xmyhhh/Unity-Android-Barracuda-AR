...
public sealed class ImageSourceSelector : MonoBehaviour
{
    ...
    // Source type options
    public enum SourceType { Texture, Video, Webcam, MoverioCam, Card, Gradient }
    ...
    // MoverioCam
    [SerializeField] RenderTexture moverioTexture;
    ...
    void Start()
    {
        ...
        if (_sourceType == SourceType.Webcam)
        {
            ...
            _webcamName = device.name;
            _webcam = new WebCamTexture(_webcamName, Screen.width, Screen.height, _webcamFrameRate);
    ...
    void Update()
    {
        ...
        if (_sourceType == SourceType.MoverioCam)
        {
            Blit(moverioTexture, flip);
        }
