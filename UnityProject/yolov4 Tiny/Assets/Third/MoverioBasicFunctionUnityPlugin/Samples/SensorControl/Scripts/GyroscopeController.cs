...
public class GyroscopeController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        var value = MoverioInput.GetGyro();
        UpdateSensorValue(value);

        try
        {
            var accuracy = MoverioInput.GetGyroAccuracy();
            UpdateSensorAccuracy(accuracy);
        }
        catch (IOException e)
        {
            Debug.LogError("Getting the accuracy of Gyroscope sensor is failed. Message = " + e.Message);
        }
    }
