...
public class AccelerometerUncalibratedController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        var value = MoverioInput.GetAccUncalibrated();
        UpdateSensorValue(value);

        try
        {
            var accuracy = MoverioInput.GetAccUncalibratedAccuracy();
            UpdateSensorAccuracy(accuracy);
        }
        catch (IOException e)
        {
            Debug.LogError("Getting the accuracy of Uncalibrated Accelerometer sensor is failed. Message = " + e.Message);
        }
    }
