...
public class GravityController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        var value = MoverioInput.GetGravity();
        UpdateSensorValue(value);

        try
        {
            var accuracy = MoverioInput.GetGravityAccuracy();
            UpdateSensorAccuracy(accuracy);
        }
        catch (IOException e)
        {
            Debug.LogError("Getting the accuracy of Gravity sensor is failed. Message = " + e.Message);
        }
    }
