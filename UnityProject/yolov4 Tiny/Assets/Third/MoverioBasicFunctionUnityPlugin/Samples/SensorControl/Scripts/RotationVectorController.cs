...
public class RotationVectorController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        var value = MoverioInput.GetRotation();
        UpdateSensorValue(value);

        try
        {
            var accuracy = MoverioInput.GetRotationAccuracy();
            UpdateSensorAccuracy(accuracy);
        }
        catch (IOException e)
        {
            Debug.LogError("Getting the accuracy of Rotation vector sensor is failed. Message = " + e.Message);
        }
    }
