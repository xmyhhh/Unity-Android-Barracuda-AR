...
public class MagneticFieldController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        var value = MoverioInput.GetMag();
        UpdateSensorValue(value);

        try
        {
            var accuracy = MoverioInput.GetMagAccuracy();
            UpdateSensorAccuracy(accuracy);
        }
        catch (IOException e)
        {
            Debug.LogError("Getting the accuracy of Magnetic field sensor is failed. Message = " + e.Message);
        }
    }
