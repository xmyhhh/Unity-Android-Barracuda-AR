...
public class LinearAccelerometerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        var value = MoverioInput.GetLinearAcc();
        UpdateSensorValue(value);

        try
        {
            var accuracy = MoverioInput.GetLinearAccAccuracy();
            UpdateSensorAccuracy(accuracy);
        }
        catch (IOException e)
        {
            Debug.LogError("Getting the accuracy of Linear Accelerometer sensor is failed. Message = " + e.Message);
        }
    }
