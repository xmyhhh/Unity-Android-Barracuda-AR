...
public class MagneticFieldUncalbratedController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        var value = MoverioInput.GetMagUncalibrated();
        UpdateSensorValue(value);

        try
        {
            var accuracy = MoverioInput.GetMagUncalibratedAccuracy();
            UpdateSensorAccuracy(accuracy);
        }
        catch (IOException e)
        {
            Debug.LogError("Getting the accuracy of Uncalibrated Magnetic field sensor is failed. Message = " + e.Message);
        }
    }
